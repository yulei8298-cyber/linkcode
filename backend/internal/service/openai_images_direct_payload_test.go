...
func TestCodexDirectImagesStreamingSuppressesRawSSELine(t *testing.T) {
	if shouldForwardOpenAIImagesRawSSELine(&OpenAIImagesRequest{Model: "gpt-image-2"}) {
		t.Fatal("direct Codex Images SSE must not forward the raw line")
	}
	if !shouldForwardOpenAIImagesRawSSELine(nil) {
		t.Fatal("legacy Responses streaming should retain raw SSE passthrough")
	}
}
