...
func openAIStreamDataStartsClientOutput(data, eventType string) bool {
	...
	case "response.output_item.added", "response.content_part.added", "response.reasoning_summary_part.added":
		// LinkCode's TTFT contract treats a structurally valid output item as
		// progress, including an empty reasoning item. This keeps the first-byte
		// metric and optional timeout failover independent of visible text.
		return true
	}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if firstTokenMs == nil && lineStartsClientOutput && trimmedData != "[DONE]" && !openAIStreamEventTypeIsTerminal(eventType) {
