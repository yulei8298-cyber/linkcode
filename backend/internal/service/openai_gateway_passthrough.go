...
// openAIStreamDataStartsStructuralProgress reports protocol progress used by
// the local TTFT contract. This intentionally mirrors the pre-v0.1.183
// LinkCode classification: output-item/content-part events count as progress
// even when their payload is empty, while retryable error frames do not.
// Client-visible output classification remains in openAIStreamDataStartsClientOutput
// so the upstream failover behavior can stay stricter where required.
func openAIStreamDataStartsStructuralProgress(data, eventType string) bool {
	...
	// Terminal events describe the outcome of the turn, not progress toward its
	// first token. In particular, a failed/incomplete response must not disarm
	// the first-output timeout merely because it is being flushed downstream.
...
// openAIStreamDataStartsVisibleOutput reports whether a Responses SSE event
// contains content that is visible/useful to the client, as opposed to mere
// protocol progress such as response.created or an empty output_item.added.
