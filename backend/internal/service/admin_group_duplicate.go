...
func cloneGroupForDuplicate(source *Group, operationID string) *Group {
	return &Group{
		...
		DailyFreeLimitUSD:               cloneGroupValuePointer(source.DailyFreeLimitUSD),
		ChatStationOnly:                 source.ChatStationOnly,
		IPWhitelist:                     append([]string(nil), source.IPWhitelist...),
		IPBlacklist:                     append([]string(nil), source.IPBlacklist...),
