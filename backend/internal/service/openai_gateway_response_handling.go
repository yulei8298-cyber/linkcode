...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	// Keep the LinkCode TTFT contract: only requests covered by the configured
	// first-output timeout use attempt-local staging. Ordinary streams record
	// the first structural output immediately.
	// Keep OpenAI attempt-local staging for every Responses stream so preamble
	// frames cannot be committed before a failover decision. The timeout guard
	// remains optional; LinkCode records TTFT from structural progress when it
	// is disabled.
	...
	completeGuardedEvent := func(queueDrained bool) {
		...
		completedFirstTokenEvent := eventStartsFirstToken
		...
		// Structural Responses events (for example an empty reasoning
		// output_item.added) prove that the upstream is making protocol
		// progress, but they are not client-visible output. They must not
		// disarm the first-output timeout/failover guard.
		if completedProgressEvent && !firstOutputProgressObserved {
			firstOutputScanGuard.Store(false)
			firstOutputProgressObserved = true
			stopFirstOutputTimer()
		}
		if completedFirstTokenEvent && firstTokenMs == nil {
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			// Keep structural TTFT independent from strict client-output/failover
			// classification, but never treat a terminal or flushed failure event as
			// progress.
			startsStructuralProgress := !openAIStreamEventTypeIsTerminal(eventType) &&
				!forceFlushFailedEvent && openAIStreamDataStartsStructuralProgress(data, eventType)
			startsFirstToken := openAIStreamDataStartsTTFT(data, eventType, forceFlushFailedEvent, ttftMode)
			if stageFirstOutput {
			...
			if guardFirstOutput {
				eventStartsFirstToken = eventStartsFirstToken || startsFirstToken
			}
			...
			if !clientDisconnected && !failureDelivered && !suppressCurrentEvent {
				...
				if firstTokenMs == nil && startsFirstToken && startsClientOutput {
			...
			if !guardFirstOutput && firstTokenMs == nil && startsFirstToken {
				ms := int(time.Since(startTime).Milliseconds())
				firstTokenMs = &ms
				stopFirstOutputTimer()
			}
