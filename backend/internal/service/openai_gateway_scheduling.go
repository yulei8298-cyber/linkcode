...
// normalizeOpenAICompatiblePlatform 保留二开代码使用的内部调用名，实际
// 复用上游新增的国产 OpenAI 兼容平台归一化规则。
func normalizeOpenAICompatiblePlatform(platform string) string {
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, error) {
	...
	if account := s.tryStickySessionHit(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, requireCompact, stickyAccountID, requiredCapability, modelRouting); account != nil {
		return account, nil
	}
	...
	selected, compactBlocked, filterStats := s.selectBestAccount(ctx, groupID, platform, accounts, requestedModel, excludedIDs, requireCompact, requiredCapability, preferLowUpstreamRate, modelRouting)
...
func (s *OpenAIGatewayService) tryStickySessionHit(ctx context.Context, groupID *int64, platform string, sessionHash, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, modelRouting openAIModelRoutingPolicy) *Account {
	if sessionHash == "" {
		return nil
	}
	platform = NormalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool, modelRouting openAIModelRoutingPolicy) (*Account, bool, openAISelectionFilterStats) {
	...
	for i := range accounts {
		...
		if !modelRouting.allows(acc.ID) {
			filterStats.exclude(modelRouting.rejectionReason())
			continue
		}
