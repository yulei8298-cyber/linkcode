...
func TestBuildAuthorizationURL_UsesGeminiCLIEnvCredentials(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientIDEnv, "test-client-id.apps.googleusercontent.com")
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "test-secret")
	...
	if !strings.Contains(authURL, "client_id=test-client-id.apps.googleusercontent.com") {
		t.Errorf("应使用环境变量中的 Gemini CLI client_id，实际 URL: %s", authURL)
	}
}

func TestBuildAuthorizationURL_MissingGeminiCLIEnvCredentials(t *testing.T) {
...
func TestEffectiveOAuthConfig_MissingGeminiCLIEnvCredentials(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientIDEnv, "")
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "")

	_, err := EffectiveOAuthConfig(OAuthConfig{}, "code_assist")
	if err == nil {
		t.Fatal("不设置 Gemini CLI OAuth 环境变量时应报错")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientIDEnv) || !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Fatalf("错误信息应提示同时配置 ID 和 Secret，实际: %v", err)
	}
