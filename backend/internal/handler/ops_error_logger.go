...
func isCountTokensRequest(c *gin.Context) bool {
	...
	return isCountTokensRequestPath(c.Request.URL.Path)
}

func isCountTokensRequestPath(path string) bool {
	return strings.Contains(path, "/count_tokens") || service.IsOpenAIResponsesInputTokensRequestPath(path)
}
...
func shouldSkipOpsErrorLog(ctx context.Context, ops *service.OpsService, message, body, requestPath string) bool {
	...
	if settings.IgnoreCountTokensErrors && isCountTokensRequestPath(requestPath) {
		return true
	}
