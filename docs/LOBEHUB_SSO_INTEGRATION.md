...
- 如果要自动生成 provider key，LinkCode 中需要存在对应平台的 active channel，并且用户有可用的 active group。
...
| LobeHub provider | LinkCode platform | 默认 key 名称 |
|---|---|---|
| Claude | Anthropic | `LobeHub Claude` |
| GPT | OpenAI | `LobeHub GPT` |
| Gemini | Gemini | `LobeHub Gemini` |
...
5. 确认 LinkCode 中 Claude、GPT、Gemini 对应渠道和用户可用分组状态正确。
...
| 用户有 active channel 和 active group | 自动创建或复用对应 provider API Key。 |
...
| 没有返回 provider key | 检查对应平台是否有 active channel，以及用户是否有该平台可用 active group。 |
