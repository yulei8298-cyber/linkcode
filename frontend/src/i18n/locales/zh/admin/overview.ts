export default {
    ...
    backup: {
      ...
      imageStorage: {
        title: '生图对象存储',
        description: '开启后，同步与异步生图结果都会转存到对象存储：Base64、data URL 和上游远程 URL 统一改写为本站配置的公开域名或预签名链接；异步任务只把短链接写入 Redis。与备份共用同一套 S3 客户端，保存后立即生效，无需重启。',
        enabled: '启用生图对象存储',
    ...
    groups: {
      ...
      form: {
        ...
        exclusive: '专属分组',
        userHidden: '对用户隐藏',
        userHiddenHint: '用户不能选择该分组创建密钥，已有该分组密钥也不会显示在用户 API 密钥列表。',
        freeGroup: '每日免费分组',
        freeGroupHint: '仅适用于标准余额模式；调用不扣用户余额，按用户和自然日累计实际成本。',
        dailyFreeLimit: '每日免费额度（USD）',
        dailyFreeLimitPlaceholder: '请输入大于 0 的每日额度',
        dailyFreeLimitRequired: '免费分组必须使用标准计费，并配置大于 0 的每日免费额度',
        chatStationOnly: '仅允许对话站请求',
        chatStationOnlyHint: '开启后仅带有正确对话站共享密钥的请求可以使用此分组。',
        nameLabel: '分组名称',
