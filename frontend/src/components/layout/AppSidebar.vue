    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M4.5 6.75A2.25 2.25 0 016.75 4.5h10.5a2.25 2.25 0 012.25 2.25v10.5a2.25 2.25 0 01-2.25 2.25H6.75a2.25 2.25 0 01-2.25-2.25V6.75z M8.25 15.75l2.25-2.25 1.5 1.5 2.25-2.25 1.5 1.5 M8.25 8.25h.008v.008H8.25V8.25z'
        })
      ]
    )
}
...
function buildCustomMenuNavItem(item: CustomMenuItem): NavItem {
  ...
  const isMarkdownPage = Boolean(item.page_slug) || url.startsWith('md:')
  return {
    path: `/custom/${item.id}`,
    label: item.label,
    icon: null,
    iconSvg: item.icon_svg,
    externalUrl: isMarkdownPage
      ? undefined
      : buildEmbeddedUrl(
          url,
          authStore.user?.id,
